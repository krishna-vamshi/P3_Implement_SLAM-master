...
class robot:
    ...
    def sense(self):
        ...
        for land_idx, landmark in enumerate(self.landmarks):
            landmark_x = landmark[0]
            landmark_y = landmark[1]
            random_noise = self.rand()
            com_x = self.x-landmark_x + random_noise* self.measurement_noise
            com_y = self.y-landmark_y + random_noise* self.measurement_noise
            measurement_range = self.measurement_range
            if((abs(com_x)<= self.measurement_range) and (abs(com_y)<= self.measurement_range)):
                measurements.append([land_idx, com_x, com_y])
